...
> Lambda Concurrency: 동시에 실행중인 람다 함수 인스턴스 개수라고 함 즉, 한 시점에 몇개 람다 함수가 동시에 실행되고 있는지 숫자로 나타낸 지표로 요청이 들어올대마다 람다는 새로운 실행 환경을 실행하거나(콜드 스타트) 기존 환경을 재사용함 따라서 트래픽이 증가하면 동시에 실행되는 람다수가 급증하고 이게 Concurrency
>
> ```
> 예: 1초 동안 200개의 요청이 들어오고,
> 각 요청의 평균 처리 시간이 0.5초이면,
> 동시에 약 100개의 Lambda가 실행됨.
> ```
>
> Concurrency가 많아지면 Quota(한도)를 넘게될수있으며 넘으면 스로틀링됨. 그래서 Provisioned Concurrency로 해결할 수 있는데 이는 콜드 스타트를 방지하는 기능임. 자세한건 aws lambda에서 알아보겟다