...
### Ah-ha Moment 발현 (지능의 자기 개척과 성찰)

GRPO + RLVR 결합 조건에서 정답률이 82.4퍼센트로 전 영역중 독보적인 수치다.

특히 주목할 점은 **평균 추론 토큰 길이가 180에서 680토큰으로 3배이상 폭증**했다는 것이다.

SFT처럼 인간의 정답 길이에 제약을 받지 않고 모델이 정답 보상 (2.0점)을 따내기 위해 스스로 여러 단계의 논리적인 검증 루틴 `<thinking>` 블록 내부를 확장하고 길게 고민하기 시작했음을 정량적으로 증명한다.

### 메모리 절감 경제성 PPO vs GRPO

PPO 환경에서는 가치 평가 모델 Critic의 적재 부담으로 인해 단일 8B 모델 튜닝에도 GPU 리소스 한계치에 직면했다.

반면, GRPO 조건에서는 그룹 상대 보상 기법 덕분에 Critic 없이 오직 2개의 모델 컴포넌트 만으로 훈련이 완료되어 **VRAM 점유율을 대폭 낮춰졌으며, 남은 가용 메모리 Rollout 배치 크기 (Group Size=8)**로 고스란히 환원하여 학습 효율을 극대화할 수 있다.

### 완벽한 Reward Hacking 방어 (RLVR의 강점)

PPO 단독 수행 시 간혹 어려운 수학 기호를 오남용하여 똑똑해보이는 문체를 생성하는 식으로 보상 모델을 속여 높은 점수를 획득하는 보상 해킹 현상이 관측되었다.

그러나 정규식 및 Python 실행 샌드박스로 구성된 RLVR(Verfier)을 보상체계로 바인딩한 후에는 아무리 미려한 문장을 작성해도 최종 `\boxed{}` 내의 값이 연산 정답과 다르면 가차 없이 0점 처리 되므로 모델이 오직 진짜 논리적 타당성을 확보하는 방향으로만 가중치가 수렴되었다.