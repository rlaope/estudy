...
자기소개는 굵게 하도록 하겠습니다. 개발자구요 네.

저는 저를 공학과 예술을 하는 사람으로 정의하고 있습니다.
...
그리고는 금융권 기업중에 이름좀있는 어떤(T) 기업에서 인턴을 잠깐 하는 기회를 얻었었구요, 지금은 AI금융 플랫폼을 서비스하는 한 기업에서 병특받고 다니고있습니다. 그리고 현재는 ai 엔지니어링을 집중적으로 공부하고 있네요.
...
취업을 하는법은 취업을 바라지 않는것인데요. 네 정확히는 취업이 아닌 본질적인 다른것에 집중해보는것입니다
...
아 이거 그냥 돈 1억 바르면 정상화 될겁니다. vs 고민은 좀 해야하는데 시스템 최적화로 이만큼 성능이 개선되고 비용절감도 1억정도 될겁니다.
...
저는 개발을 할때 추구하는 방향성이 간헐적으로 100점차리 코드를 뱉기보단 꾸준하게 80점 짜리 코드를 뱉고 품질을 유지하며 나머지 팀원들에게 해당 부분을 채워주도록

하는 방향성을 추구하고 있는데요. 이런겁니다. 여기 비즈니스 로직을 살펴보겠습니다 A라는 테이블이 있고 A라는 테이블을 A_new로 마이그레이션 한상황이고

로직도 새로 작성한 상황입니다. 근데 여기서 A ID 123이라는 친구는 죽어도 A_new를 사용할수없을정도로 구조가 깨져있어 123친구는 A를 그대로 이용하는 로직을 태우고싶습니다.

어떻게 해야될까요? A라는 친구를 A_new로 마이그레이션하려면 진짜 많은 리소스가 들수있으며 시간도 오래걸릴것같습니다.

근데 여기엔 제약사항이 있습니다 A 데이터는 한달뒤면 더이상 사용안하고 만료될 데이터라는 제약사항이 있다고 쳐보고 내일 당장 오픈을 해야된다고 해봅시다.

구구조 신구조 로직은 다 작성되어있구요. 그럼 간단하죠?
...
여러분 일단 지금 이순간만큼은 개발과 취업에 대해서 잠시 벗어나보자구요.
...
그리고 저희 심장을 뛰게하는건 예술입니다. 기술적인 기계의 동작을 예술적이라고 느끼는 사람은 엔지니어로서 충분한 역량을 가진거고요

제가 추구하는 예술과 기술의 조합을 컴퓨터에서 모두 느끼니 적성이라고 보시면 되겠습니다. 여러분들은 아니라구요?

그럼 뭐 어때요? 예술은 하나만 있는것이 아니구요 다른 분야랑 결합될수도 있습니다. 다른 분야가 금융일수도 패션일수도 방송일수도 글쓰기일수도 음식일수도 있어요.

게임일수도 있고 애니메이션일수도 있습니다. 이런게 아예없다 취미생활은 없고 나는 그냥 안식적인 안정적인 삶을 살고... 네 키워드 나왔네요 안정적인, 안식적인
...
그 누구보다 상대성 이론에 대해 오래 고민했기 때문이라고 전 생각해요. 그니까 인풋이 우리보다 많으니까 아웃풋이 많은거죠.

인간은 성장할 수 있고요 그렇게 설계된 동물이에요, 왜 그런지는 잘 모르겠지만 인간은 생각보다 멍청하지만 가능성은 무궁무진합니다.
