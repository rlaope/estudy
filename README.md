...
엔지니어의 추월차선이라는 개념을 요즘 스스로 구상하고 있는데, **기록(記錄)이 규모의 계명을 타파할 수 있을까** 자문하기도 합니다.
...
내용이 다르든 구조가 다르든 형식이 다르든 어찌되었든 저장소속에서는 모두가 소중하고 멋진 데이터들이다.  
말이 길어졌다. 어찌되었든 이 저장소는 단순하게 *소프트웨어 개발자로서 잡다한 가치를 지닌 기록들의 집합*이다.  
어느 내용은 당신에게 도움이 되지 않을지도 모르며 어느 내용은 터닝포인트가 될 수도 있지 않을까 싶다.  
(※ 물론 개념 위주의 기록이 대부분이라 터닝포인트 트리거는 어려울 듯 싶긴합니다. 교양과 인문에 대한 정리는 차차 채워가려 합니다.)
